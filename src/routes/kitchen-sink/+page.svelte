...
<Section>
  ...
  <p class="type-body-lg text-deep/70">Every ported design-system primitive, for visual review.</p>
</Section>
...
<Section tone="white">
  ...
  <div class="flex flex-col gap-4">
    {#each buttonVariants as v (v)}
      <div class="flex flex-wrap items-center gap-3" class:bg-green={v === 'inverse'} style={v === 'inverse' ? 'padding:1rem;border-radius:1rem' : ''}>
    ...
    {/each}
    <div><Button onclick={() => toastStore.push('Toast fired', 'success')}>Fire a toast</Button></div>
  </div>
</Section>
...
<Section>
  ...
  <div class="grid gap-6 sm:max-w-md">
    ...
    <Textarea label="Notes" name="notes" optionalNote="optional" placeholder="Anything else?" />
    <Select
      id="cat"
      ariaLabel="Category"
    ...
    />
    <Checkbox label="Add a gift note" description="Handwritten at pickup" bind:checked price="+$1.50" />
    <RadioGroup
      label="Fulfilment"
      name="fulfil"
      bind:value={radio}
      options={[
        { value: 'pickup', label: 'Store pickup', description: 'Cash on collection' },
...
<Section>
  ...
  <EmptyState message="Your cart is empty" hint="Add some groceries to get started.">
...
<Section>
  ...
  <div class="grid gap-4 sm:grid-cols-3">
    <MenuItemCard name="Jollof rice mix" description="500g" price="$6.99" tags={['New']} onAdd={() => {}} />
    <MenuItemCard name="Plantain chips" price="$2.49" onAdd={() => {}} />
    <MenuItemCard name="Scotch bonnet" price="$3.99" soldOut onAdd={() => {}} />
  </div>
  <div class="mt-4 flex flex-col gap-3">
    <MenuItemCard layout="row" name="Egusi seeds" description="1kg bag" price="$12.00" onAdd={() => {}} />
    <MenuItemCard layout="row" name="Palm oil" description="1L" price="$8.50" soldOut onAdd={() => {}} />
  </div>
</Section>
...
<Section>
  ...
  <Breadcrumbs items={[{ label: 'Home', href: '/' }, { label: 'Shop', href: '/shop' }, { label: 'Egusi seeds' }]} />
  <Card class="mt-4 sm:max-w-md">
    <CartLine name="Egusi seeds" quantity={2} price="$24.00" onEdit={() => {}} onRemove={() => {}} />
    <CartLine name="Palm oil" quantity={1} price="$8.50" onRemove={() => {}} />
  </Card>
</Section>
...
<Modal open={modalOpen} title="Example modal" onclose={() => (modalOpen = false)}>
  <div class="p-4">
...
<Drawer open={drawerOpen} title="Example drawer" onclose={() => (drawerOpen = false)}>
