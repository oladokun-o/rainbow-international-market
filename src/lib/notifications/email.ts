...
function wrap(title: string, text: string): string {
  const body = text
    ...
    .join('<br />');
  return `<!doctype html><html><body style="margin:0;background:#fbf7ef;padding:24px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#12351f;">
  <div style="max-width:520px;margin:0 auto;background:#ffffff;border:2px solid rgba(18,53,31,0.1);border-radius:16px;padding:28px;">
    <p style="margin:0 0 16px;font-size:13px;letter-spacing:0.14em;text-transform:uppercase;color:#1e6b3f;font-weight:700;">Rainbow International Market</p>
    <h1 style="margin:0 0 16px;font-size:20px;color:#1e6b3f;">${escapeHtml(title)}</h1>
